...
class TestPreflightChecks:
    ...
    def test_ubuntu_iptables_flushed(self, ssh_client, extras, controller_profile):
        """
        Ubuntu 24.04: check iptables rules.
        radm init flushes iptables automatically so this is informational only.
        Auto-flushes rules if count > 10 to ensure clean install.
        """
        if controller_profile.os_type != "ubuntu24":
            pytest.skip("iptables flush check is Ubuntu 24.04 only")
        out, rc = ssh_client.run("sudo iptables -L | wc -l")
        attach_output(extras, "iptables rule count", out)
        rule_count = int(out.strip())

        attach_output(extras, "iptables rule count before", str(rule_count))

        if rule_count > 10:
            print(f"[test_ubuntu_iptables_flushed] {rule_count} rules found — flushing ...")
            flush_out, flush_rc = ssh_client.run(
                "sudo iptables -F && "
            ...
            attach_output(extras, "iptables flush result", flush_out)
            # Verify after flush
            after_out, _ = ssh_client.run("sudo iptables -L | wc -l")
            attach_output(extras, "iptables rule count after flush", after_out.strip())
            print(f"[test_ubuntu_iptables_flushed] after flush: {after_out.strip()} rules")
        else:
            attach_output(extras, "iptables status", f"Clean — {rule_count} rules (no flush needed)")
            print(f"[test_ubuntu_iptables_flushed] iptables clean ({rule_count} rules)")
...
class TestPackageSetup:
    ...
    def test_aria2c_installed(self, ssh_client, controller_profile, extras):
        """
        Ensure aria2c is installed on the VM.
        aria2c supports 16 parallel connections which significantly speeds up
        large package downloads (~4GB controller tar).

        Install strategy (Ubuntu 24.04):
          1. apt-get update && apt-get install -y aria2
          2. If apt fails (package not in repo) -- download static binary from GitHub
        Install strategy (RHEL):
          1. yum install -y aria2
        """
        ...
        if controller_profile.os_type == "ubuntu24":
            # Step 1: apt-get update first (OCI images may have stale package lists)
            update_out, _ = ssh_client.run("sudo apt-get update -y 2>&1", timeout=120)
            attach_output(extras, "apt-get update", update_out)

            install_out, install_rc = ssh_client.run(
                "sudo apt-get install -y aria2 2>&1", timeout=180
            )
            attach_output(extras, "apt install aria2", install_out)

            if install_rc != 0:
                # Step 2: fallback -- download static aria2c binary from GitHub releases
                attach_output(extras, "apt failed", "Falling back to static binary download ...")
                fallback_cmd = (
                    "cd /tmp && "
                    "sudo curl -sL https://github.com/aria2/aria2/releases/download/release-1.37.0/"
                    "aria2-1.37.0-aarch64-linux-android.tar.bz2 -o aria2.tar.bz2 2>&1 || "
                    # x86_64 build
                    "sudo curl -sL https://github.com/q3aql/aria2-static-builds/releases/download/v1.37.0/"
                    "aria2-1.37.0-linux-gnu-64bit-build1.tar.bz2 -o aria2.tar.bz2 2>&1"
                )
                # Simpler: build from snap
                snap_out, snap_rc = ssh_client.run(
                    "sudo snap install aria2c 2>&1 || "
                    "sudo apt-get install -y aria2c 2>&1 || "
                    # Last resort: wget-based workaround using wget with parallel via xargs
                    "echo SNAP_FAILED",
                    timeout=120
                )
                attach_output(extras, "snap install attempt", snap_out)

                if snap_rc != 0 or "SNAP_FAILED" in snap_out:
                    # Final fallback: install from universe repo
                    ...
                    assert universe_rc == 0, (
                        "aria2c could not be installed via apt, snap, or universe repo. "
                        "SSH into the VM and run: sudo add-apt-repository universe && sudo apt install aria2"
                    )
        else:
            # RHEL 8 / RHEL 9
            out, rc = ssh_client.run(
        ...
        attach_output(extras, "aria2c version (post-install)", verify_out)
        assert verify_rc == 0, (
            "aria2c still not found after all install attempts. "
            "SSH into the VM and install manually: sudo apt install aria2"
        )
    ...
    def test_package_download(self, ssh_client, package_profile, nsg_manager, extras):
        ...
        download_ok = False
        try:
            # Confirm NSG attached
            if nsg_manager:
                nsg_manager.attach()
                attach_output(extras, "NSG status", f"Attached: {nsg_manager.profile.nsg_id}")
                print(f"[test_package_download] NSG attached: {nsg_manager.profile.nsg_id}")
            else:
                attach_output(extras, "NSG status", "No nsg_manager -- ensure VM has internet access")

            # Wait for NSG rules to propagate
            print("[test_package_download] Waiting 10s for NSG rules to be active ...")
            time.sleep(10)
    ...
    def test_config_yaml_created(self, ssh_client, package_profile, request,
        ...
        patches = [
            ...
            f"sudo sed -i '/type:/s|:.*|: {q}airgap{q}|' {config_path}",
            # archive-directory handled via Python below (path contains / chars)
        ]
...
class TestRadmInstall:
    ...
    def test_radm_init_completes(self, ssh_client, package_profile,
        ...
        if already_initialized:
            ...
            # Deep clean all stale k8s state before running init
            print("[test_radm_init_completes] Deep cleaning stale k8s state ...")
            ssh_client.run(
                ...
                "sudo rm -rf /tmp/rafay-infra && "
                "sudo systemctl restart containerd && "
                "sleep 3 && "
                "echo CLEANUP_DONE",
                timeout=60
            )
            attach_output(extras, "k8s cleanup", "Stale state removed")
            print("[test_radm_init_completes] Cleanup complete")

            print(f"[test_radm_init_completes] Step 1: running radm init on node1 ...")
            out, rc = ssh_client.run(
                f"cd {extract_dir} && sudo ./radm init --config config.yaml 2>&1",
                timeout=1800,
            )
