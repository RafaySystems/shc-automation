...
class TestPackageSetup:
    ...
    def test_dns_resolver_configured(self, ssh_client, extras):
        ...
        if "MISSING" in check_out or "nameserver" not in check_out:
            print("[test_dns_resolver_configured] resolv.conf missing — fixing ...")

            # Step 1: try resolvconf symlink (standard on OCI Ubuntu VMs)
            ssh_client.run(
                "sudo systemctl start resolvconf 2>/dev/null || true && sleep 1",
                timeout=10
            )
            ...
            print(f"[test_dns_resolver_configured] resolvconf: {fix1_out.strip()}")

            # Step 2: try systemd-resolved symlink
            if "RESOLVCONF_MISSING" in fix1_out:
                ssh_client.run(
                    "sudo systemctl start systemd-resolved 2>/dev/null || true && sleep 2",
                    timeout=15
                )
                ssh_client.run(
            ...
            # Step 3: fallback — remove broken symlink and write nameservers directly
            recheck, _ = ssh_client.run(
                "cat /etc/resolv.conf 2>/dev/null || echo STILL_MISSING"
            )
            if "nameserver" not in recheck:
                print("[test_dns_resolver_configured] Writing resolv.conf directly ...")
                ssh_client.run(
            ...
            print(f"[test_dns_resolver_configured] resolv.conf: {final_out.strip()[:80]}")
        else:
    ...
    def test_setup_secondary_nodes(self, ssh_client, package_profile,
        ...
        Per secondary node:
          1. ufw disable + iptables flush
          2. NSG attach
          3. apt-get update + install aria2c (IPv4 forced)
          4. iptables re-flush after apt
          5. Download package (NSG stays attached)
          6. NSG detach
          7. Extract package
          8. Copy config.yaml from node1
        """
        if not controller_profile.ha or not secondary_ips:
        ...
        # Pad secondary_instance_ids with empty strings if not provided
        # zip() stops at shortest list — pad so all IPs are iterated
        padded_ids = list(secondary_instance_ids) + [""] * len(secondary_ips)

        for i, (sec_ip, sec_id) in enumerate(
            zip(secondary_ips, padded_ids), 2
        ):
            ...
            try:
                # 1. Create install dir with write permissions for ubuntu user
                sec_ssh.run(
                    f"sudo mkdir -p {package_profile.install_dir} && "
                    f"sudo chmod 777 {package_profile.install_dir}"
                )

                # 2. ufw disable + iptables flush (same as primary node preflight)
                sec_ssh.run(
                    "sudo ufw disable 2>/dev/null || true && "
                    "sudo systemctl stop ufw 2>/dev/null || true && "
                    "sudo systemctl disable ufw 2>/dev/null || true && "
                ...
                attach_output(extras, f"node{i} firewall", "ufw disabled + iptables flushed")
                print(f"[test_setup_secondary_nodes] node{i}: firewall disabled")
                ...
                if not already_extracted:
                    # 3. NSG attach for internet access
                    if sec_nsg:
                        sec_nsg.attach()
                        print(f"[test_setup_secondary_nodes] node{i}: NSG attached — waiting 30s ...")
                        time.sleep(30)
                        attach_output(extras, f"node{i} NSG", "Attached")

                    # 4. Install aria2c — force IPv4 to avoid OCI IPv6 timeout
                    aria2c_out, aria2c_rc = sec_ssh.run(
                        "which aria2c 2>/dev/null && aria2c --version 2>/dev/null | head -1"
                    )
                    if aria2c_rc != 0 or "aria2" not in aria2c_out.lower():
                        print(f"[test_setup_secondary_nodes] node{i}: installing aria2c ...")
                        sec_ssh.run(
                            "sudo apt-get -o Acquire::ForceIPv4=true update -y 2>&1 || true",
                            timeout=120
                        )
                        sec_ssh.run(
                            "sudo apt-get -o Acquire::ForceIPv4=true install -y aria2 2>&1 || true",
                            timeout=180
                        )
                        sec_ssh.run(
                            "sudo iptables -F && sudo iptables -t nat -F && "
                        ...
                        print(f"[test_setup_secondary_nodes] node{i}: aria2c installed, iptables re-flushed")
                    else:
                        print(f"[test_setup_secondary_nodes] node{i}: aria2c already installed")

                    # 5. Download package via aria2c (16 parallel connections)
                    ...
                    if "MISSING" in tar_check:
                        ...
                        print(f"[test_setup_secondary_nodes] node{i}: downloading package ...")
                    ...
                    # 6. Detach NSG — internet no longer needed
                    if sec_nsg:
                        sec_nsg.detach()
                        attach_output(extras, f"node{i} NSG", "Detached after download")
                ...
                # 8. Copy patched config.yaml from node1
...
class TestRadmInstall:
    ...
    def test_radm_init_completes(self, ssh_client, package_profile,
        ...
        print(f"[test_radm_init_completes] Step 3: HA={controller_profile.ha} secondary_ips={secondary_ips}")
        if controller_profile.ha and not secondary_ips:
            print("[test_radm_init_completes] WARNING: HA=true but secondary_ips is empty — "
                  "add secondary_ips to dev.yaml or pass --secondary-ips flag")
        if controller_profile.ha and secondary_ips:
            ...
            # Build HA control-plane join command from radm init output
            # For HA, secondary nodes join as control-plane masters, not workers.
            # Requires 3 extra values vs worker join:
            #   --control-plane
            #   --certificate-key  (from radm init output, valid for 2 hours)
            # Find rafay-kubeadm binary — radm installs it to
            # /tmp/rafay-infra/packages/kubeadm/amd64/rafay-kubeadm
            kubeadm_out, _ = ssh_client.run(
                "find /tmp/rafay-infra /usr/local/bin /usr/bin "
                "-name 'rafay-kubeadm' -type f 2>/dev/null | head -1"
            )
            kubeadm_bin = kubeadm_out.strip() or "/tmp/rafay-infra/packages/kubeadm/amd64/rafay-kubeadm"
            print(f"[test_radm_init_completes] kubeadm binary: {kubeadm_bin}")

            # Get bootstrap token (authentication,signing) or create a new one
            token_out, _ = ssh_client.run(
                f"sudo {kubeadm_bin} token list "
                f"--kubeconfig=/etc/kubernetes/admin.conf 2>/dev/null | "
                f"grep 'authentication,signing' | head -1 | awk '{{print $1}}'"
            )
            if not token_out.strip():
                print("[test_radm_init_completes] No valid bootstrap token — creating new one ...")
                token_out, _ = ssh_client.run(
                    f"sudo {kubeadm_bin} token create "
                    f"--kubeconfig=/etc/kubernetes/admin.conf 2>/dev/null"
                )
                print(f"[test_radm_init_completes] New token: {token_out.strip()}")
            ...
            # Re-upload certs and get a fresh certificate key
            # Output format: "Using certificate key:" followed by 64-char hex on next line
            cert_key_raw, _ = ssh_client.run(
                f"cd {extract_dir} && "
                "sudo ./radm init phase infra upload-certs --config config.yaml 2>&1"
            )
            attach_output(extras, "upload-certs output", cert_key_raw[-500:])
            # Extract the 64-char hex key
            cert_key = ""
            for line in cert_key_raw.splitlines():
                ...
                if len(stripped) == 64 and all(c in "0123456789abcdef" for c in stripped):
                    cert_key = stripped
                    break
            print(f"[test_radm_init_completes] cert_key extracted: {'YES' if cert_key else 'NO'}")
            ...
            assert cert_key, "Could not get certificate key — needed for control-plane join"

            print(f"[test_radm_init_completes] token    : {token}")
            print(f"[test_radm_init_completes] pri_ip   : {pri_ip}")
            print(f"[test_radm_init_completes] cert_key : {cert_key}")

            # HA control-plane join — includes --control-plane and --certificate-key
            join_cmd = (
                f"cd {extract_dir} && "
            ...
            attach_output(extras, "radm join command (HA control-plane)", join_cmd)

            # Wait 60s before joining secondary nodes
            # etcd needs time to stabilize after radm init before accepting new members
            # Joining too early causes "can only promote a learner" etcd error
            import time as _join_wait
            print("[test_radm_init_completes] Waiting 60s for etcd to stabilize before join ...")
            _join_wait.sleep(60)

            import subprocess as _subprocess
            import time as _time
            for i, sec_ip in enumerate(secondary_ips, 2):
                print(f"[test_radm_init_completes] Joining node{i} ({sec_ip}) via system ssh ...")

                # Pre-join cleanup on secondary node:
                # 1. Stop consul — stale consul intercepts k8master.service.edgedc.consul DNS
                # 2. Add /etc/hosts entry — radm join hardcodes consul DNS lookup
                prereq_cmd = [
                    "ssh",
                    "-i", controller_profile.ssh_key,
                    "-o", "StrictHostKeyChecking=no",
                    "-o", "UserKnownHostsFile=/dev/null",
                    "-o", "ConnectTimeout=30",
                    f"{controller_profile.user}@{sec_ip}",
                    # Stop consul (was acting as DNS on port 53)
                    ...
                    # Fix resolv.conf — consul stopping may leave it empty
                    # kubelet reads from resolvConf: /run/systemd/resolve/resolv.conf
                    # Must write to ALL resolv.conf paths to cover all cases
                    ...
                    # Add /etc/hosts entry for consul DNS lookup in radm join
                    f"grep -q 'k8master.service.edgedc.consul' /etc/hosts || "
                    f"echo '{pri_ip} k8master.service.edgedc.consul' | sudo tee -a /etc/hosts && "
                    f"echo PREREQ_DONE"
                ]
                prereq_result = _subprocess.run(prereq_cmd, capture_output=True, text=True, timeout=30)
                print(f"[test_radm_init_completes] node{i}: pre-join prep: {prereq_result.stdout.strip()}")

                # Use system ssh binary for radm join — paramiko hangs on open_session()
                # for long-running commands on some OCI VMs
                ssh_join_cmd = [
                    "ssh",
                    "-i", controller_profile.ssh_key,
                    "-o", "StrictHostKeyChecking=no",
                ...
                # Retry up to 3 times — etcd learner promotion can fail transiently
                max_join_attempts = 3
                join_rc = 1
                join_out = ""
                for attempt in range(1, max_join_attempts + 1):
                    if attempt > 1:
                        print(f"[test_radm_init_completes] node{i}: retrying join (attempt {attempt}/{max_join_attempts}) ...")
                        _time.sleep(30)
                    ...
                    if join_rc == 0:
                        break
                    print(f"[test_radm_init_completes] node{i}: join attempt {attempt} failed (exit {join_rc})")
    ...
    def test_ha_nodes_ready(self, ssh_client, controller_profile, extras):
        """
        HA mode only: verify all 3 control-plane nodes are in Ready state.
        Runs after radm init + radm join completes.
        Skipped for Non-HA.
        """
        ...
        import time
        max_wait  = 600  # 10 minutes — control-plane nodes need time to start etcd+apiserver
        poll      = 15
        deadline  = time.time() + max_wait

        print("[test_ha_nodes_ready] Waiting for all 3 control-plane nodes to be Ready ...")

        while time.time() < deadline:
            nodes_out, rc = ssh_client.run(
                "kubectl get nodes --no-headers 2>/dev/null || "
                "/usr/local/bin/kubectl get nodes --no-headers 2>&1"
            )
            lines = [l for l in nodes_out.splitlines() if l.strip()]
            ready = [l for l in lines if "Ready" in l and "NotReady" not in l]
            total = len(lines)
            print(f"[test_ha_nodes_ready] {len(ready)}/{total} nodes Ready ...")

            if len(ready) >= 3:
                attach_output(extras, "All nodes Ready", nodes_out)
                print(f"[test_ha_nodes_ready] All 3 nodes Ready ✓")
                return
        ...
        assert not not_ready, (
            f"Not all 3 control-plane nodes are Ready after {minutes} min: " + nodes_out
        )
